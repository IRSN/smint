...
!
    ...
    integer NPTS(40)
    integer LNEXT(N), LCELL(NR, NR, NR)

    double precision AV, AVSQ, C, DMIN
    double precision DTOL
    double precision FK, RMAX, RQ, RS, RSMX, RSOLD
    double precision RTOL
    PARAMETER(RTOL = 1.0E-05)
    double precision RWS, S
    double precision SF
    PARAMETER(SF = 1.0E+00)
    double precision SUM2, T, XK, YK, ZK
    double precision F(N), RSQ(N), X(N)
    double precision XYZDEL(3), XYZDL(3), XYZMIN(3), XYZMN(3)
    double precision Y(N), Z(N)
    double precision A(9,N)
    double precision B(10, 10)

    DTOL = SQRT(EPSILON(1.0D00))
    NN = N
    NNQ = NQ
    NNW = NW
    NNR = NR
...
!
            ...
            IF (.NOT.( RQ /= 0.0E+00  .OR.  LNP <= NNQ )) THEN
!
...
!
    ...
    double precision DELX, DELY, DELZ, XMN, XMX, YMN, YMX, ZMN, ZMX
    double precision X(N), Y(N), Z(N)                    
    double precision XYZDEL(3), XYZMIN(3)            
...
!
    ...
    XYZDEL(2) = DELY
    XYZDEL(3) = DELZ
    IF (DELX == 0.0E+00 .OR. DELY == 0.0E+00 .OR. &
          DELZ == 0.0E+00) THEN
        IER = 2
        RETURN
    END IF
...
!
    ...
    double precision DELX, DELY, DELZ, DSQ, DX, DY, DZ, PX, PY, &
...
!
    IF (NR < 1 .OR. DX <= 0.0E+00 .OR. DY <= 0.0E+00 & 
          .OR. DZ <= 0.0E+00) THEN
        NP = 0
        DSQ = 0.0E+00
        RETURN
...
!
    ...
    double precision D, DX, DXSQ, DY, DYSQ, DZ, DZSQ, FI, FK, R, S1, S2, &
...
!
    ...
    integer I, IER, IMAX, IMIN, J, JMAX, JMIN, K, KMAX, KMIN, L, LP, N, NR
    integer LNEXT(N), LCELL(NR,NR,NR)
    
    double precision DELX, DELY, DELZ, DS, DX, DXSQ, DY, DYSQ, DZ, DZSQ, &
    ...
    double precision F(N), RSQ(N), X(N), Y(N), Z(N)
    double precision XYZDEL(3), XYZMIN(3) 
    double precision A(9,N)
...
!
    ...
    integer I, IMAX, IMIN, J, JMAX, JMIN, K, KMAX, KMIN, L, LP, N, NR
    integer LNEXT(N), LCELL(NR,NR,NR)
    
    double precision DELX, DELY, DELZ, DS, DX, DXSQ, DY, DYSQ, DZ, &
