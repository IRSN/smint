...
colors can be of some help, see later.
...
immaterial in the computation of \verb@f2@ above.
...
levels from data frame representation is quite tedious, since it
...
frame and in the list, and the data frame contains as assumed a tensor
product of finite sets.

In the \pkg{smint} package, it was decided to define a S4 class for the
grid under the name \verb@Grid@, in the aim that standard methods
give the frequently needed information. As in the data frame
representation, the nodes will be considered as ordered so a
\verb@Grid@ object can be thought of as a \textit{traveller salesman's
path} visiting each node of the grid exactly once.
...
\noindent
Note that the default dimnanes are \verb@"X1"@, \verb@"X2"@ and so on,
but can be specified by using a named vector for the number of
levels. They can also be changed by using the replacement method
\verb@dimnames<-@ as illustrated before.
...
\verb@nlevels@. Note that a \verb@Grid@ object with moderate
...
\verb@GriData@ stores all the $n$ possible combinations of the levels
as vectors of length~$d$ with \textit{integer} values rather than
double. So for a large grid, the \verb@Grid@ representation needs only
...
method. Combined with semi-transparent colors, a improved
...
\noindent
See figure~\ref{plotdfGrid}.  The points can be given different colors,
size, ... by using vectors.  Remind then that the order of points is
then essential.
...
this on purpose by using the \verb@drop@ function. A similar operation
...
second dimension $x_2$ (or $y$).  This form is often require to
produce contour plot or a perspective plot (not shown here).  The
